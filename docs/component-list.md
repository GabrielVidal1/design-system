...
Every component and hook in the library (v0.30.1), with a one-line description of what it is for. Skim this before building UI: if something here already covers the need, use it instead of writing a new component.
...
| **[RichInput](https://ui.gabvdl.xyz/#/c/rich-input)** | `RichInput` · `defaultComposePrompt` · `useSavedDrafts` | The full composer: draft persistence (text + tag selection), a saved-drafts shelf (long-press send to stash, fuzzy-search dropdown to restore), un-send window, file attachments, guideline tags with inline search, `#mention` autocomplete, prompt history, and an optional hold-to-rearrange toolbar (reorder the controls, bench spares in a stash). Use for any chat/agent input. |
